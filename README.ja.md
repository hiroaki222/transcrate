...
[**ダウンロード**](https://github.com/hiroaki222/transcrate/releases/latest) は Apple Silicon 向けの `.dmg` と Windows 向けの `.exe` です。どちらも ffmpeg を同梱しているので、ほかに用意するものはありません。

![曲を並べて、どの機材で鳴るかを一覧している画面](docs/images/convert.ja.png)

曲ごとに機材 10 台分のランプが、いつも同じ順番で並びます。再生できないランプには斜線が入るため、色を見分けられなくても読めます。再生を保証しない変換先を選んだときは、下の段に変換後の判定が出ます。保証する 2 つを選んだときは付け足すことがないので、下の段は出しません。

コマンドライン版もあります。判定に使う対応表は、メーカーの説明書から取ったものです。
...
公式の情報同士が食い違う箇所 (XDJ-XZ の exFAT) は、どちらかを選ばずに、食い違っていることをそのまま表示します。アプリ側は再生できない側に倒して、不可とだけ出します。資料の矛盾を現場で確かめる方法はないからです。

## アプリ

[ダウンロード](https://github.com/hiroaki222/transcrate/releases/latest)して開き、曲かフォルダをドラッグ&ドロップするだけです。

有料の開発者証明書は取得していません。年 99 ドルかかりますし、まだ誰も使っていない段階で払う気になれませんでした。そのため macOS も Windows も初回起動時に警告を出しますが、一度許可すれば次からはそのまま開きます。macOS の手順は Apple が案内しています ([身元不明の開発者による Mac App を開く][unsigned-mac])。Windows では SmartScreen が **詳細情報 → 実行** を求めてきます。

画面は 3 つあります。

- **CONVERT** — 各行に、今の形式、変換後の形式、そして機材 10 台分のランプが並びます。緑が再生できる機材、赤い斜線が再生できない機材です。手をかける必要がある行にだけ印を付けるので、40 曲並んでいても印の付いた行だけ見れば済みます。元から 192 kbps を下回る曲には別の注意が付きます。再生自体はできますが、最初のエンコーダが捨てた情報は変換しても戻りません。

  ![曲ごとに、今の判定と変換後の判定が並んだ画面](docs/images/convert.ja.png)

- **USB CHECK** — 挿さっているメディアの一覧から選ぶと、まずどの機材が読めるかを判定します。続けて中の曲をすべて読み、機材が受け付けないものを名指しします。フォルダの構造も測ります。機材が降りられるのはフォルダ 8 階層まで、1 フォルダに並べられるのは 10,000 件までです。どちらかを超えると、メディアは認識され、曲もそこにあるのに、機材の画面には何も出てきません。読み取れなかったフォルダも名指しします。数え漏らしがある間は、問題なしとは集計しません。読み取り専用で、初期化のボタンは置いていません。
...
こちらは PATH の通った ffmpeg が必要です。同梱しているのはアプリ版だけです。

macOS と Linux では Homebrew が近道です。ffmpeg も一緒に入ります。

```sh
brew install hiroaki222/tap/transcrate
```

それ以外は、リリースページから自分の環境向けの圧縮ファイルを落としてください。中身はバイナリ 1 つで、インストーラは付いていません。ファイル名に対象の環境が入っています。
...
macOS ではダウンロードしたバイナリに隔離属性が付き、初回実行が理由も告げずに拒否されます。一度だけ外してください。
...
フォルダを渡しても `*` をシェルに展開させても、拾うのは音声ファイルだけです。ジャケット画像やプレイリストはエラーにならず、そのまま無視されます。前回の実行で作られた `_transcrate` フォルダも除外するため、二度実行しても出力を再変換することはありません。

ただしパスを 1 つだけ指定した場合は、拡張子に関係なく必ず処理を試みます。ファイルを 1 つだけ指定したなら、そのファイルを処理したいのは明らかだからです。判定も、拡張子より ffprobe の方が正確です。
...
フォルダごと渡したときは、その形のまま出力します。フォルダの隣に `_transcrate` を 1 つ作り、中の曲は読み込んだときと同じ深さに書き出します。ファイルを 1 つずつ指定したときは保つべき形がないので、結果はそれぞれの元ファイルの隣に置きます。

元ファイルには一切書き込みません。出力先が衝突する入力、たとえば同じフォルダの `mix.wav` と `mix.flac` はどちらも `mix.mp3` を作ろうとしますが、この 2 つは両方とも拒否します。片方を黙って上書きさせるよりましだからです。

すでに目的の形式になっているファイルは、再エンコードせずコピーします。速いうえに、ロッシー音源を二度潰さずに済みます。ロッシー音源を元より高いビットレートで再エンコードすることもありません。128 kbps の曲を `cdj-safe` にかけても 128 kbps のまま出ます。同じ音をエンコーダに二度通すと、音は悪くなって容量だけ 2.5 倍になります。
...
ここでの失敗は、指定した機種の**いずれか 1 つでも**再生できないことです。10 機種のうち 9 機種で鳴っても、残り 1 機種が現場にあればセットは止まります。

全機種で再生できても、知っておく価値のある曲があります。
...
パスを付けずに実行すると、接続されているメディアを一覧します。Mac のマウント先パスなんて覚えていられません。
...
ファイルシステムの判定に続けて、中の曲を 1 曲ずつ ffprobe で読みます。時間がかかるのはこの読み取りなので、`--no-tracks` を付けるとファイルシステムの判定だけで止まります。フォルダの構造も測ります。機材はフォルダ 8 階層までしか降りず、1 フォルダに 10,000 件までしか並べません。どちらかを超えると、メディアは認識され、曲もそこにあるのに、機材の画面には何も出てきません。

権限がない、途中でメディアが抜けたなどで開けなかったフォルダは、黙って飛ばさずに名指しします。そのあとの集計も、メディア全体ではなく見つかった曲のうちの数だと明示します。読み切れなかった実行は終了コードを 0 以外にします。

**読み取り専用です。** ドライブへの書き込み、フォーマット、ファイルの移動は一切行いません。自分のセットに向けて使うツールが、それを壊せる必要はありません。

### タグとアートワーク

元ファイルが持っていたタグはそのまま引き継ぎます。ただし `lyrics-eng` は空にします。歌詞を CDJ で読む人はいませんし、リッピングツールが宣伝文句を書き込む場所でもあります。タイトル・アーティスト・アルバム・ジャンル・キー・BPM は残します。ブラウザで曲を探すのに必要な情報だからです。

コメントも残します。ここは配信サイトが宣伝文句を書き込む場所で、CDJ はそれをブラウザのタイトルの隣に表示します。消したくもなりますが、自分でキューのメモや Camelot キーを書き込んでいると、消したぶんは戻りません。消したいときは `--clear-comment` を付けてください。
...
- **AIFF の muxer は、指定しない限り ID3 チャンクを書きません。** アートワークも一緒に失われます。タイトルとアーティストは AIFF 独自のチャンクに残るので、症状はジャケットだけが出ない形になり、タグが落ちたことに気づきにくくなります。このフラグは有効にしてあります。
...
各ファイルは元の形式のまま出力するので、MP3 と AIFF が混在したフォルダでも、拡張子ごとにコマンドを分ける必要はありません。音声ストリームはそのままコピーします。ロッシー音源が文字列の書き換えで劣化することはなく、すでに正しい音声を再エンコードする時間もかかりません。
...
Rust 1.88 以降と、PATH の通った ffmpeg が必要です。チェックアウトした状態には同梱物がないので、PATH で見つかった `ffmpeg` を使います。自前でビルドした ffmpeg を試したいときは、そのまま差し替えられます。
...
最後の 1 つは CI では別ジョブで、いちばん忘れやすいものです。画面側の型を検査するので、import が壊れていても上の 3 つは通り、ここで初めて落ちます。
...
アプリ版のリリースには **LGPL** ビルドの ffmpeg を実行ファイルの隣に同梱しています。GPL ビルドは使いません。このプログラムは MIT / Apache-2.0 なので、同じバンドルに GPL のバイナリを入れると、配布物側にも GPL の義務が及ぶためです。LGPL ビルドでも、書き出しに使う形式はすべて賄えます。MP3 は libmp3lame、AAC は ffmpeg 自身のエンコーダ、FLAC / ALAC / PCM は本体機能です。macOS 向けの LGPL ビルドは公開されていません。Windows 向けに公開されているものはフルビルドで、切り詰めたものが 1 実行ファイルあたり 4 MB なのに対して 115 MB あります。それがダウンロードのたびに付いてくるので、[リリース時に両方をソースからビルドしています](.github/scripts/build-ffmpeg.sh)。対応形式の一覧は共通の定義にまとめ、GPL 専用の部品は外してあります。
