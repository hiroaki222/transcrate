...
曲ごとに機材 10 台分のランプが、いつも同じ順番で並びます。再生できないランプには斜線が入るため、色を見分けられなくても読めます。再生を保証しない変換先を選んだときは、下の段に変換後の判定が出ます。保証する 2 つを選んだときは言うことが残らないので、下の段は出しません。
...
- **CONVERT** — 各行に、今の形式、変換後の形式、そして機材 10 台分のランプが並びます。緑が再生できる機材、赤い斜線が再生できない機材です。手をかける必要がある行にだけ印を付けるので、40 曲並んでいても読まずに走査できます。元から 192 kbps を下回る曲には別の注意が付きます。再生自体はできますが、最初のエンコーダが捨てた情報は変換しても戻りません。

  ![曲ごとに、今の判定と変換後の判定が並んだ画面](docs/images/convert.ja.png)

- **USB CHECK** — 挿さっているメディアの一覧から選ぶと、まずどの機材が読めるかを判定します。続けて中の曲をすべて読み、機材が受け付けないものを名指しします。フォルダの構造も測ります。機材はフォルダ 8 階層までしか降りず、1 フォルダに 10,000 件までしか並べないため、どちらかを超えると、メディアは認識され、曲もそこにあるのに、機材の画面には何も出てきません。読み取れなかったフォルダも名指しします。数え漏らしがある間は、集計を「問題なし」とは表示しません。読み取り専用で、初期化のボタンは置いていません。
...
フォルダごと渡したときは、その形のまま出力します。フォルダの隣に `_transcrate` を 1 つ作り、中の曲は読み込んだときと同じ深さに書き出します。ファイルを 1 つずつ指定したときは保つべき形がないので、結果はそれぞれの元ファイルの隣に置かれます。

元ファイルには一切書き込みません。出力先が衝突する 2 つの入力 — 同じフォルダの `mix.wav` と `mix.flac` はどちらも `mix.mp3` を要求します — は、片方が黙って上書きするのではなく、両方とも拒否します。

すでに目的の形式になっているファイルは、再エンコードせずコピーします。速いうえに、ロッシー音源を二度潰さずに済みます。ロッシー音源を元より高いビットレートで再エンコードすることもありません。128 kbps の曲を `cdj-safe` にかけても 128 kbps のまま出ます。同じ音をエンコーダに二度通せば音は悪くなり、容量は 2.5 倍になるためです。
...
権限がない、途中でメディアが抜けたなどで開けなかったフォルダは、黙って飛ばさずに名指しします。そのあとの集計は「見つかった曲のうち」であってメディア全体ではないことを明示し、読み切れなかった実行は終了コードを 0 以外にします。
