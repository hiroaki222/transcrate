...
> ### Which file
>
> The downloads are under **Assets**, below this.
>
> **macOS** — the one ending in **`.dmg`**. Apple silicon only.
> **Windows** — the one ending in **`.exe`**.
>
> Both carry their own ffmpeg, so nothing else needs installing. The `transcrate-…` archives are the command line tool, which expects an ffmpeg you installed yourself.
>
> ---
>
...
> ### どれをダウンロードすればいいか
>
> ファイルはこの下の **Assets** にあります。
>
> **macOS** — 拡張子が **`.dmg`** のもの。Apple Silicon 専用です。
> **Windows** — 拡張子が **`.exe`** のもの。
>
> どちらも ffmpeg を同梱しているので、ほかに用意するものはありません。`transcrate-` で始まる書庫はコマンドライン版で、こちらは自分で入れた ffmpeg を使います。
>
> ---
>
> ### 初回起動時に警告が出たら
